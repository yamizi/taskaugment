...
from utils.metrics import success_sort_all
from utils.metrics import common_success_metrics

def run(parameters, name="topk_accuracy", random_seed=0, experiment=None, topk=3):
    ...
    for i, batch in enumerate(loader):
        ...
        direction = parameters.get("direction", "indirect")
        ...
        if parameters.get("target")=="random":
            ...
            classes_to_sort = perm[:, 0:topk]
        elif parameters.get("target")=="reverse":
            _, topk_values = clean_output.topk(topk, 1, True, True)
            classes_to_sort = topk_values
        ...
        with torch.no_grad():
            output = m(adv)
            sorted_acc = success_sort_all(output,classes_to_sort.transpose(0,1))
            sorted_acc_original = success_sort_all(clean_output, classes_to_sort.transpose(0, 1))
            experiment.log_metric("sortall_success", sorted_acc)
            experiment.log_metric("sortall_original_success", sorted_acc_original)

        common_success_metrics(experiment, clean_output, output, labels, None)

        print(sorted_acc_original, sorted_acc)

def runner(parameters, name="rq1_sortall", topk=None):
    print("SortAll")

    topk = parameters.get("topk", 3) if topk is None else topk
    combinations = [{"criterion":"ord_ce"}, {"criterion":"ord"},
                    {"criterion":"mse"}, {"criterion":"bce"}]

    targets = ["reverse","random"]
...
if __name__ == '__main__':
    parameters = {"use_hidden": False, "pretrained": True, "criterion": "mse", "algorithm": "PGD", "max_eps": 8/255,
                  ...
                  "workspace":"SortedAttacks", "target":"random", "topk":3}
