...
if __name__ == '__main__':
    ...
    comet_path = cfg.weights_file.split("/")

    api = api.API(COMET_APIKEY)
    xps = api.get(comet_path[0], comet_path[1])

    for xp in xps:

        params = xp.get_parameters_summary()
        attributes = {k["name"]: k["valueCurrent"] for k in params}

        weight_file = attributes.get("save_path")
