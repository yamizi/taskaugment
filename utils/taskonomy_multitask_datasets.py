...
from utils.taskonomy.task_data_loading import load_target
from torchxrayvision.datasets import Dataset

class TaskonomyLoader(Dataset):
    NUM_CLASSES = 21
    ...
    def __init__(self,
        ...
        for i, (where, subdirs, files) in enumerate(
                os.walk(os.path.join(root, 'rgb'))):  # there is a folder rgb and images are stored there
            print(where)
        ...
        for task in task_set:
            import importlib;
            config = importlib.import_module('dataloaders.new.configs.{}.config'.format(task))
    ...
    def __getitem__(self, index):
        ...
        ys['img'] = ys['rgb'] = im
        ys["masks"]  = masks
        if ys.get("class_object"):
            ys["lab"] = ys.get("class_object")


        if self.transform is not None:
            ys["img"] = self.transform(ys["img"])

        if self.data_aug is not None:
            ys["img"] = self.data_aug(ys["img"])

        return self.enrich(ys, index)
