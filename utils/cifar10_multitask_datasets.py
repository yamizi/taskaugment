...
class CIFAR10_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if "ss" in augment:
            ...
            self.img_dataset = load_cifar10artificial(data_dir=data_folder, train=train)
            self.img_dataset.targets = self.img_dataset.tensors[1]
            self.img_dataset.data = torch.movedim(self.img_dataset.tensors[0], 3, 1).numpy()
        ...
        if data_subset < 1 and not "gowal" in augment:
            ...
            if hasattr(self.img_dataset, "tensors"):
                self.img_dataset.tensors = (
                self.img_dataset.tensors[0][:nb_items], self.img_dataset.tensors[1][:nb_items])
