### Extended from https://github.com/AntixK/PyTorch-Model-Compare  - Anand Subramanian - MIT Licence
...
class CKA:
    ...
    def compare(self,
        ...
        num_batches = min(len(dataloader1), len(dataloader2)) if max_batches==0 else max_batches
        for cur_batch, data1, data2 in tqdm(zip(range(num_batches),dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):

            x1 = data1.get("img") if isinstance(data1, dict) else data1[0]
            x2 = data2.get("img") if isinstance(data2, dict) else data2[0]
            ...
            from MedicalRobustTraining.utils import cka as cka_fn
            #cka_matrix = cka_fn()


            for i, (name1, feat1) in enumerate(self.model1_features.items()):
                if isinstance(feat1, dict):
                    feat1 = feat1[list(feat1.keys())[1]]
                X = feat1.flatten(1)
                K = X @ X.t()
        ...
        assert not torch.isnan(self.hsic_matrix).all(), "HSIC computation resulted in NANs"
