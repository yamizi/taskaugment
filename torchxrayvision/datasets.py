...
class Dataset():
    def __init__(self):
        self.include_sex = False
        self.include_age = False
        self.include_hog = False
        self.include_ae = False
    ...
    def set_includes(self, cfg):

        self.cuda = cfg.cuda

        if hasattr(cfg,"loss_sex"):
            self.include_sex = cfg.loss_sex !=0
            self.include_age = cfg.loss_age != 0
            self.include_hog = cfg.loss_hog != 0
            self.include_ae = cfg.loss_ae != 0
            self.include_depth = cfg.loss_depth != 0


    def enrich(self, sample, idx):
        if self.include_age:
            sample["age"] = self.csv["Age"].iloc[idx]

        if self.include_sex:
            sex = [0,1] if self.csv["Sex"].iloc[idx] =="M" else [1,0]
            sample["sex"] = np.asarray(sex).T.astype(np.float32)

        if self.include_ae:
            sample["autoencoder"] = torch.clone(sample["img"].detach())

        if self.include_hog:
            from skimage.feature import hog

            _, hg = hog(np.moveaxis(sample["img"].numpy(),0,2), orientations=self.orientations,
            ...
            sample["hog"] = torch.from_numpy(hg).unsqueeze(0)
    ...
    def check_paths_exist(self):
        ...
        if not os.path.isdir(self.imgpath):
            raise Exception("imgpath {} must be a directory".format(self.imgpath))
        if not os.path.isfile(self.csvpath):
            raise Exception("csvpath {} must be a file".format(self.csvpath))
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        item = self.dataset[self.idxs[idx]]
        return item
...
class NIH_Dataset(Dataset):
    ...
    def __init__(self, imgpath, 
        ...
        self.csv["Sex"] = self.csv['Patient Gender']
        age = self.csv['Patient Age'].values
        self.csv["Age"] = [int(a.replace("Y","")) if "Y" in a else int(a[:-1])//12 for a in age]
...
class PC_Dataset(Dataset):
    ...
    def __init__(self, imgpath, 
        ...
        self.csv = self.csv[(self.collectDate-self.csv.PatientBirth > 10)]
        ...
        self.csv["offset_day_int"] = dt.values.astype(int)// 10**9 // 86400
        self.csv["Age"] = self.collectDate - self.csv["PatientBirth"]
        self.csv["Sex"] = self.csv["PatientSex_DICOM"]
...
class CheX_Dataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.csv["Sex"] = self.csv["Sex"].astype(str).str[0]
