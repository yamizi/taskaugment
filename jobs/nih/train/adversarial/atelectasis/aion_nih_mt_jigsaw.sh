...
CUDA_VISIBLE_DEVICES=0 python experiments/train_xrayvision.py --labelfilter "Atelectasis" --loss_jigsaw 1 --attack_target "Atelectasis" --batch_size 32 -name train-nih-adv --max_eps 4  --steps 4  --weight_strategy ${1:-ew} --force_cosine ${2:-0} --algorithm ${3:-MADRY} --dataset nih --model multi_task_resnet50 --dataset_dir  /mnt/lscratch/users/sghamizi/datasets/datasets --output_dir  /mnt/lscratch/users/sghamizi/models/xray_adv --num_epochs 250 --threads $THREADS  --fine_tune $FINE
