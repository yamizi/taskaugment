...
MODEL="wide2810"
NAME="adv_baseline_wideresnet28"
OUTPUTDIR="/mnt/lscratch/users/yletraon/models/baseline_mgda/wide2810/$DATASET"
DATASETDIR="/mnt/lscratch/users/yletraon/datasets/datasets"
WEIGHT=${1:-mgda}
EVALATK=${2:-PGD}
TRAINATK=${3:-MADRY}
PREFIX="aux_cifar10_train-multi_task_wide2810-train-wide2810-madry-"
BATCHSIZE=256

#VANILLA
ATTACKTASKS="multilabel"
SUFFIX="_adv-multilabel_optim-sgd_cosine-0_w-ew_a-${TRAINATK}-best"
CUDA_VISIBLE_DEVICES=0 python  "experiments/test_model.py" -name "${NAME}_${EVALATK}" --dataset $DATASET --steps 10 --record_all_tasks 0 --img_size 32  --labelfilter "$MAIN" --model "multi_task_$MODEL" --record_roc 0 --weights_file "best/${MODEL}/${PREFIX}${MAIN}${SUFFIX}.pt" --batch_size $BATCHSIZE --attack_target $ATTACKTASKS  --dataset_dir $DATASETDIR --output_dir $OUTPUTDIR  --algorithm ${EVALATK}


SUFFIX="_adv-multilabel_optim-sgd_cosine-0_w-${WEIGHT}_a-${TRAINATK}-best"
