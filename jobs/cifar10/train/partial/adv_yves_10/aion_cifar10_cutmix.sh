...
CUDA_VISIBLE_DEVICES=0 python experiments/train_baseline.py --labelfilter "class_object#multilabel" -name train-baseline-rebuttal-partial  --attack_target "multilabel"  --data_subset $DATASUBSET --optimizer "sgd"  --lr 0.1  --cutmix 1  --dataset aux_cifar10_train --batch_size 256 --nb_secondary_labels 4 --model multi_task_resnet50  --force_cosine ${2:-0} --algorithm ${3:-MADRY}  --dataset_dir  $DATASETDIR --output_dir  $OUTPUTDIR  --num_epochs 250 --seed 10
